...
import { getYearRatings, upsertRating, exportYear } from '@/routes/api/day-ratings'
...
export default function YearGrid({ year }: { year: number }) {
    ...
    ;(async () => {
      try {
        ...
        for (const r of rows) {
          ...
          map[day] = { day, rating: r.rating ?? null, note: r.note ?? null }
        }
        setRatings(map)
      } catch (err) {
        console.error(err)
  ...
  const handleSave = async ({ day, rating, note }: { day: string; rating: number | null; note?: string | null }) => {
    ...
    <div>
      <div ref={gridRef} className="grid grid-cols-7 gap-1">
        {dates.map((d) => {
          ...
          const r = ratings[d]
          return (
            <DaySquare key={d} dateStr={d} rating={r?.rating ?? null} isFuture={isFuture} onClick={handleClick} />
          )
        })}
      </div>
